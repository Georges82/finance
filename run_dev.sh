...
run_backend() {
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        osascript <<EOF
tell application "Terminal"
    do script "cd '$BACKEND_DIR' && . venv/bin/activate && uvicorn app:app --reload"
    activate
end tell
EOF
    elif [[ "$OSTYPE" == "linux-gnu"* ]]; then
        # Linux
        if command_exists gnome-terminal; then
            gnome-terminal --tab --title="Backend" -- bash -c "cd '$BACKEND_DIR' && . venv/bin/activate && uvicorn app:app --reload; exec bash"
        elif command_exists xterm; then
            xterm -title "Backend" -e "cd '$BACKEND_DIR' && . venv/bin/activate && uvicorn app:app --reload; bash" &
        else
            print_warning "Could not detect terminal. Please run the backend manually in a new terminal:"
            print_warning "cd $BACKEND_DIR && . venv/bin/activate && uvicorn app:app --reload"
        fi
    else
